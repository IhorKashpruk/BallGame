...
void createBasicContext() {
    ...
    auto* afterGameContext = new ModernContext<AfterGameLogic>("after_menu_context");
    ...
    // Create game context
    auto* gameContext = new ModernContext<GameLogic>("game_context");

    // Create world context
    pt::Rectangle<int> rectangle{{window::size.w/2, window::size.h/2}, 0, {window::size.w/2,window::size.h/2}};
    auto* worldContext =
            new WorldContext<WorldLogic, pt::Rectangle>("world_context", rectangle);
    gameContext->add(worldContext);

    // Create menu context
    auto* menuContext = new ModernContext<MenuLogic>("menu_context");
